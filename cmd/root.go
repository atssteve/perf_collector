...
func Execute() {
	...
	rootCmd.Flags().DurationVarP(&newAgent.MetInterval, "metinterval", "i", time.Duration(2)*time.Second, "The number of seconds to wait before collecting metrics.")
	rootCmd.Flags().DurationVarP(&newAgent.ConfInterval, "confinterval", "c", time.Duration(5)*time.Second, "The number of seconds to wait before collecting config.")
